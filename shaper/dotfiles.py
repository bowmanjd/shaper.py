...
def ssh_agent_info() -> dict:
    ...
    agent_vars = dict(s.strip().split("=") for s in shell_script.split(";") if s.strip().startswith("SSH"))
    return agent_vars


def ssh_ensure_agent_loaded() -> None:
    try:
        subprocess.check_call(["ssh-add", "-l"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    except subprocess.CalledProcessError:
        os.environ.update(ssh_agent_info())
        subprocess.check_call(["ssh-add"])


def dotfiles_ssh(repo: str, ssh_prefix: str="git@github.com:", http_prefix: str="https://github.com/") -> None:
    """Get SSH dotfiles.
